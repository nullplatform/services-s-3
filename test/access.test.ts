...
describe("link create", () => {
  test("creates the user, its scoped policy and one access key", async () => {
    ...
    const run = await pkg.run(
      connect,
      "create",
      { access_level: "read", path_prefix: "uploads/" },
      { service, link, providers },
    );
    expect(run.status).toBe("success");
    expect(run.results).toEqual({
      aws_access_key_id: "AKIAEXAMPLE",
      aws_secret_access_key: "s3cr3t",
      iam_user_name: "np-s3-9f2a1b3c4d5e6f70",
    });
    expect(iam.commandCalls(CreateUserCommand)[0].args[0].input.Tags).toEqual([
      ...
      { Key: "bucket", Value: "np-orders-api-uploads" },
    ]);
    ...
    expect(policy.Statement[1]).toMatchObject({
      Action: ["s3:GetObject"],
      Resource: "arn:aws:s3:::np-orders-api-uploads/uploads/*",
    });
    expect(run.steps.map((s) => s.key)).toEqual([
      "user np-s3-9f2a1b3c4d5e6f70",
      "policy read on uploads/",
      "access key",
    ]);
  });
  ...
  test("before the bucket exists the link is retryable", async () => {
    const run = await pkg.run(
      ...
      { service: { ...service, attributes: {} }, link, providers },
    );
...
describe("link update", () => {
  test("rewrites the policy and keeps the credentials", async () => {
    ...
    const run = await pkg.run(
      ...
      "update",
      { access_level: "write" },
      { service, link: { ...link, attributes: { iam_user_name: "np-s3-custom", path_prefix: "" } }, providers },
    );
    expect(run.status).toBe("success");
    expect(run.results).toEqual({});
    expect(iam.commandCalls(PutUserPolicyCommand)[0].args[0].input.UserName).toBe("np-s3-custom");
    expect(
      JSON.parse(iam.commandCalls(PutUserPolicyCommand)[0].args[0].input.PolicyDocument!).Statement[1].Action,
    ).toEqual(["s3:PutObject", "s3:DeleteObject", "s3:AbortMultipartUpload"]);
    expect(iam.commandCalls(CreateAccessKeyCommand)).toHaveLength(0);
  });
});
...
describe("link delete", () => {
  test("removes policy, keys and user; the bucket is untouched", async () => {
    ...
    const run = await pkg.run(
      ...
      "delete",
      {},
      { service, link: { ...link, attributes: { iam_user_name: "np-s3-9f2a1b3c4d5e6f70" } }, providers },
    );
    expect(run.status).toBe("success");
